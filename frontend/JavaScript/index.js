...
function initCreateMapDefault() {
  ...
  if (!_maps.create) {
    ...
    _maps.create.on('click', async e => {
      ...
      if (label) { label.textContent = '📍 Obtendo endereço…'; label.style.display = 'block'; }
      ...
      await _reverseGeocodeFillAddr(e.latlng.lat, e.latlng.lng, label);
    });
  } else {
...
function getGPS() {
  ...
  navigator.geolocation.getCurrentPosition(async pos => {
    ...
    await _reverseGeocodeFillAddr(parseFloat(lat), parseFloat(lng), label);
  }, err => toast('Erro GPS: ' + err.message, 'error'), { enableHighAccuracy: true, timeout: 10000 });
}

// Preenche o campo de endereço (c-addr) e o rótulo visível a partir de lat/lng
// via geocodificação reversa (Nominatim) — mesmo padrão usado na landing page.
async function _reverseGeocodeFillAddr(lat, lng, labelEl) {
